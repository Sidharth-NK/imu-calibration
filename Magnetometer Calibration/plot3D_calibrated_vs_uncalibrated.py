...
Calibration parametrers were determined by the Magneto calibration software 
...
A = np.array(   [[0.145823, 0.000991, -0.000419],
                [0.000991, 0.147375, -0.000204],
                [-0.000419, -0.000204, 0.219387]])
b = np.array([-74.885685, 71.982943, -26.272430])


filename = "magnetometer_log4.txt"
...
norms = np.linalg.norm(calibData, axis=1).reshape(-1, 1)
calibData_normalized = calibData / norms

plt.figure(figsize=(6, 6))
plt.scatter(calibData_normalized[:, 0], calibData_normalized[:, 1], c='green', s=8, alpha=0.7)
plt.title('Normalized Calibrated Magnetometer Data (XY)')
plt.xlabel('X')
plt.ylabel('Y')
plt.axis('equal')
plt.grid(True)
